...
for region in api.regions():
    ...
    for route in api.routes(region.ID):
        print ('\tROUTE:', route.Name, '(R: '+str(route.ID)+')')
        print('\t\tSTOPS:')
        for stop in api.stops(route.ID):
            print ('\t\t\t', stop.Name, '(S: '+str(stop.ID)+')')
        print('\t\tVEHICLES:')
        for veh in api.vehicles(route.ID):
            print ('\t\t\t', veh.Latitude, ',', veh.Longitude, '(V: '+str(veh.Name)+')')
